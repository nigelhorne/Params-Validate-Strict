...
subtest "Valid Inputs" => sub {
	...
	my $validated_params2 = validate_strict($schema, $params2);
	...
	throws_ok { $validated_params3 = validate_strict($schema, $params3) }
...
subtest "Invalid Inputs" => sub {
	...
	my $params1 = { username => "sh" }; # Too short
	my $validated_params1 = eval { validate_strict($schema, $params1) };
	like $@, qr/username/, "Short username should fail";

	my $params2 = { username => 'x' x 51 }; # Too long
	my $validated_params2 = eval { validate_strict($schema, $params2) };
	like $@, qr/username/, "Long username should fail";

	my $params3 = { age => "-1" }; # Invalid age
	my $validated_params3 = eval { validate_strict($schema, $params3) };
	like $@, qr/age/, "Invalid age should fail";

	my $params4 = { email => "invalid_email" }; # Invalid email
	my $validated_params4 = eval { validate_strict($schema, $params4) };
	like $@, qr/email/, "Invalid email should fail";

	my $params5 = { price => "-1" }; # Invalid price
	my $validated_params5 = eval { validate_strict($schema, $params5) };
	like $@, qr/price/, "Invalid price should fail";

	my $params6 = { quantity => "0" }; # Invalid quantity
	my $validated_params6 = eval { validate_strict($schema, $params6) };
	like $@, qr/quantity/, "Invalid quantity should fail";

	my $params7 = { password => "password" }; # Invalid password
	my $validated_params7 = eval { validate_strict($schema, $params7) };
	like $@, qr/password/, "Invalid password should fail";

	my $params8 = { name => { 'value' => 123 } }; # Invalid name (should be a simple string)
	my $validated_params8 = eval { validate_strict($schema, $params8) };
